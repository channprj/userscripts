...
| JumpCloud Login Assistant | 암호 관리자가 입력한 로그인 폼을 ID/PW 원문 접근 없이 자동 진행합니다. | 0.1.0 | [Install](https://raw.githubusercontent.com/channprj/userscripts/main/jumpcloud-login-assistant.user.js) |
...
`https://console.jumpcloud.com/login#/`의 **User Login**에서 암호 관리자가 입력한 이메일과 비밀번호 단계를 자동으로 진행합니다. 스크립트에는 ID/PW를 입력하는 설정이 없으며, 입력 필드의 원문도 읽지 않습니다. 암호 보관·자동 입력은 브라우저/암호 관리자가, MFA는 사용자가 맡습니다.

#### 로컬 암호 보관과 최초 설정

로컬 전용 구성이 필요하면 **Chrome에 로그인하지 않은 별도 프로필의 장치 암호 저장소**를 사용할 수 있습니다. Chrome 공식 문서는 브라우저에 로그인하지 않았을 때 암호가 장치에 로컬 저장된다고 설명합니다. Google 계정 저장과 구분하세요. [Chrome 암호 관리 안내](https://support.google.com/chrome/answer/95606?hl=en)

1. 사용할 브라우저 프로필에서 암호의 저장 위치를 확인합니다. **클라우드 계정에 저장하거나 동기화하는 설정은 로컬 전용 요구사항에 맞지 않습니다.** 기존 동기화를 끄더라도 이미 클라우드에 저장된 암호가 삭제됐다는 의미는 아닙니다.
2. JumpCloud 주소가 정확한지 확인하고 **직접 한 번 로그인**해 ID/PW를 해당 암호 저장소에 저장합니다. ID/PW를 코드, `.env`, GM 설정, 이 저장소의 파일에 적지 마세요. 평문 CSV로 내보내거나 가져오는 과정도 필요 없습니다.
3. 로그인 화면에서 암호 관리자가 원하는 계정을 자동 입력하는지 확인합니다. 여러 계정이 있다면 사용할 계정을 직접 선택합니다. 스크립트는 계정 원문을 읽지 않으므로 계정 선택이나 이메일·비밀번호의 계정 일치 여부를 판단하지 않습니다. JumpCloud의 **Remember me**에 기억된 계정도 확인하세요.
4. 위 **Install** 링크로 스크립트를 설치한 뒤 로그인 화면을 새로고침합니다. 처음에는 자동 진행이 꺼져 있습니다.
5. Tampermonkey / Violentmonkey 메뉴에서 **JumpCloud Login Assistant: 자동 진행 켜기/끄기**를 선택합니다. 활성화 여부는 다음 접속에도 유지됩니다.
6. 로그인 탭을 활성화한 상태에서 자동완성을 기다립니다. 필수 입력과 버튼 상태가 1초 동안 준비되어 있으면 공식 **Continue / Login** 버튼을 한 번씩 누릅니다. 암호 관리자 잠금 해제나 계정 선택이 필요하면 직접 완료합니다.
7. MFA가 나타나면 직접 인증합니다. **비밀번호 단계를 제출했습니다**는 성공 확인이 아니므로 실제 포털 진입 여부도 확인하세요.

브라우저 암호 관리자를 이미 사용하는 경우 보관 위치를 확인한 뒤 3번부터 진행할 수 있습니다. 외부 암호 관리자는 폼을 한 번에 채우고 사이트의 입력 이벤트를 갱신하는 방식이면 사용할 수 있지만, 제품별 동작을 모두 검증하지는 않았습니다. 키보드로 한 글자씩 입력하는 도구는 수동 입력으로 인식되어 자동 진행이 중지될 수 있습니다. 1Password나 Apple 암호를 사용한다는 사실만으로 로컬 전용 보관이 보장되지는 않으며, 해당 제품의 계정·동기화 설정을 별도로 확인해야 합니다.

#### 동작 제어

| 매니저 메뉴 | 동작 |
| --- | --- |
| **자동 진행 켜기/끄기** | 사용 여부를 저장하고 현재 페이지에서 적용합니다. |
| **현재 페이지 중지** | 현재 실행을 중지합니다. 다음 새 페이지에서는 저장된 활성화 설정을 따릅니다. |
| **다시 시도** | 자동 진행이 켜져 있을 때 제출 제한 기록을 초기화하고 현재 화면부터 다시 시작합니다. 오류나 계정을 먼저 확인하세요. |
| **상태 안내** | 닫은 안내를 다시 표시합니다. ID/PW는 표시하지 않습니다. |

페이지 안내의 **중지** 또는 `Esc`도 현재 실행을 중단합니다. **닫기**는 안내만 숨깁니다. 직접 타이핑·붙여넣기·수동 제출을 감지하면 자동 진행을 중지하므로, 수동 입력 후에는 직접 로그인하거나 **다시 시도**를 선택하세요. 다른 로그인 탭이 열려 있다면 그 탭에서도 중지하거나 새로고침해 변경한 설정을 반영하세요.
...
스크립트는 입력값, 폼 직렬화, 이벤트의 입력 데이터, 페이지 HTML/텍스트 전체, Vue 내부 상태, 쿠키를 읽지 않습니다. 자체 네트워크 요청과 외부 라이브러리도 없습니다. 기존 로그인 버튼을 눌렀을 때의 인증 요청은 JumpCloud 페이지가 처리합니다.

| 저장 위치 | 보관 내용과 담당 |
| --- | --- |
| 사용자가 설정한 암호 저장소 | ID/PW. 저장과 자동 입력은 암호 관리자가 수행하며 userscript는 접근하지 않습니다. |
| userscript 매니저 저장소 | `enabled` boolean만 저장합니다. |
| JumpCloud 출처의 전용 localStorage 키 | `chann.jumpcloud-login-assistant.attempts.v1`에 이메일/비밀번호 단계의 제출 시각만 저장합니다. 계정이나 입력값은 없습니다. |

전용 제출 기록은 Web Locks 안에서 읽고 갱신합니다. JumpCloud가 자체적으로 저장한 계정·세션 키는 조회하지 않습니다. 제출 기록은 같은 출처 페이지가 읽거나 변경할 수 있는 비민감 중복 방지용 데이터이며, 인증 보안 수단은 아닙니다. 삭제하려면 개발자 도구의 저장소 화면에서 **해당 키만** 지우면 됩니다. 스크립트 제거는 암호 저장소에 있는 ID/PW를 삭제하지 않으므로, 암호 삭제는 암호 관리자에서 별도로 수행하세요.

**이 스크립트가 ID/PW를 읽거나 저장하지 않는 것과, DOM에 입력된 ID/PW를 모든 스크립트로부터 격리하는 것은 다릅니다.** 일반 웹 로그인 폼에 입력된 값은 페이지 코드나 다른 확장 프로그램에서 접근할 수 있습니다. 이보다 강한 격리가 필요하면 조직이 지원하는 패스키/JumpCloud Go 같은 비밀번호 없는 인증을 검토해야 합니다. [JumpCloud의 로그인 방식](https://jumpcloud.com/support/get-started-user-portal)

#### 검증 범위

공개 로그인 화면의 DOM 속성과 공개 로그인 번들의 비밀번호 컴포넌트를 확인했습니다. 가상 계정으로 작성한 테스트 58개에서 원문 접근 금지, 단계 진행, MFA/오류 중단, 입력 취소, 탭 경쟁, 저장 실패와 반복 제출 제한을 검증했습니다. 실제 계정으로 암호 관리자 자동완성 → MFA → 포털 진입까지의 통합 검증은 수행하지 않았습니다. 설치 후 위 최초 설정 절차로 확인하세요.
...
JumpCloud 스크립트 테스트는 입력 원문 getter, 폼 직렬화, 로그, 쿠키, 네트워크, 페이지 전체 텍스트/HTML 접근을 차단한 가상 DOM에서 실행합니다. Web Locks, 암호 관리자 자동 입력, 매니저 설정, 전용 제출 기록만 테스트 경계에서 재현합니다. 실제 브라우저 암호 저장소를 읽지 않습니다.
