...
| Google Search Navigator | Google 검색 결과와 이미지 탭을 Vim 스타일 단축키로 탐색합니다.              | 0.12 | [Install](https://raw.githubusercontent.com/channprj/userscripts/main/google-search-navigator.user.js) |
...
Google 검색 결과와 이미지 탭을 키보드만으로 빠르게 탐색합니다.
...
이미지 탭에서는 `H` / `J` / `K` / `L` 또는 방향키로 이미지를 하나씩 이동합니다.
