# JumpCloud Login Assistant 0.4.0 설계

## 목표와 변경된 보안 계약

`https://console.jumpcloud.com/login*`의 User Login에서 쿼리·해시와 관계없이 Tampermonkey에 저장한 계정으로 이메일 → 비밀번호 단계를 진행한다. 사용자의 잠금 암호 제거 요청에 따라 ID/PW만 한 번 등록하고 이후 활성화된 새 로그인 페이지에서 자동 진행한다. MFA는 직접 완료한다.
...
최근 제출 기록에 따른 대기 제한은 없다. 제출 시각 저장·조회와 Web Locks 의존성을 제거했으며, 새 페이지와 명시적 다시 시도는 즉시 실행할 수 있다.

한 계정만 저장한다. 기본 상태는 비활성화이며 설정의 **저장 후 자동 로그인**으로 활성화한다. 알려진 이메일·비밀번호 로그인 폼만 자동 제출한다. 비밀번호 재설정·가입·MFA 폼과 다른 출처의 인증 페이지는 자동화하지 않는다. 다른 userscript의 동작은 변경하지 않는다.
...
| localStorage·sessionStorage | 접근하지 않음. 이전 제출 기록도 조회·변경하지 않음. |
...
1. 최상위 프레임, 정확한 `https://console.jumpcloud.com` 출처와 `/login`으로 시작하는 경로를 확인한다. 메타데이터의 `https://console.jumpcloud.com/login*`와 일치하는 범위다. 쿼리·해시는 파싱하거나 실행 조건으로 사용하지 않는다. 같은 범위 내의 URL 변경도 실행을 중단하지 않는다.
...
7. 같은 동기 실행에서 경로·활성 상태·폼·MFA·알림·계정·설정을 확인하고 공식 버튼을 클릭한다. 클릭 전에 현재 실행의 `sent` 집합에 단계를 표시해 페이지 이벤트가 다시 발생해도 같은 단계를 중복 제출하지 않는다. 비동기 잠금이나 제출 시각 기록은 사용하지 않는다.
8. 한 실행에서 이메일·비밀번호 단계를 각각 한 번만 자동 제출한다. 새로고침·새 탭·명시적 다시 시도는 별도 실행이며 이전 제출 시각에 따른 제한이 없다. 여러 페이지는 독립적으로 진행한다.
...
10. 중지는 GM의 ID/PW를 삭제하지 않는다. **다시 시도**는 활성화된 경우 현재 실행을 정리하고 저장 정보로 즉시 재개한다. 다음 새 로그인 페이지도 활성화 설정을 따르며 잠금 해제는 필요하지 않다.

이전 버전의 `chann.jumpcloud-login-assistant.attempts.v1` 기록은 유효 여부·시각과 관계없이 무시한다. 삭제나 변환 없이 업데이트 후 바로 실행할 수 있다. 비밀번호 단계를 제출한 뒤 실제 로그인 성공으로 추정하지 않으며 MFA와 결과는 사용자가 확인한다.
...
JumpCloud 92개와 전체 190개 테스트, 문법·공백 검사가 통과했다. 쿼리·해시·경로 접미사, 단계 사이 URL 변경, 최근·미래·손상된 이전 제출 기록, 새로고침·독립 탭, Web Locks·localStorage 미지원 조건을 검증했다. 설정·계정 일치·수동 제출·MFA 중단과 실행 내 반복 클릭 방지 회귀도 유지했다. 모든 테스트에서 localStorage 읽기·쓰기와 Web Locks 접근이 없는지 확인한다.
